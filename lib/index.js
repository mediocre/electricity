...
exports.static = function static(directory, options) {
    ...
    function cacheFile(filePath, stat) {
        ...
        if (!stat.isDirectory()) {
            ...
            if (ext === '.scss' && !shouldIgnore(relativeUrl, options.sass.ignore)) {
                ...
                data = sass.renderSync(options.sass);
    ...
    function hashifyCss(css) {
        return css.toString().replace(/url\(['"]?(.*?)['"]?\)/g, function(match, filename) {
            var file = files[filename];
            if (file) {
                return 'url("' + hashifyPath(filename, files[filename].hash) + '")'; // Add hash
            } else {
    ...
    function reloadFile(filePath, stat) {
        ...
        if (stylesheets.indexOf(relativeUrl) != -1) {
            files[relativeUrl].content = hashifyCss(files[relativeUrl].content);
        }
    }
    ...
    stylesheets.forEach(function(stylesheet) {
        files[stylesheet].content = hashifyCss(files[stylesheet].content);
    });
    ...
    if (options.watch.enabled) {
        watch.createMonitor(directory, function monitorFiles(monitor) {
            ...
            monitor.on('removed', function removeFile(f, stat) {
                compiledPath = f.replace(/\.scss$/, '.css');
