...
exports.static = function static(directory, options) {
    ...
    function cacheFile(filePath, stat) {
        ...
        if (!stat.isDirectory()) {
            ...
            if (ext === '.css' && options.uglifycss.enabled) {
                // Uglifycss
                data = data ?
                    UglifyCss.processString(data, options.uglifycss) :
                    UglifyCss.processFiles([filePath], options.uglifycss);
            } else if (ext === '.js' && options.uglify.enabled && !shouldIgnore(relativeUrl, options.uglify.ignore)) {
                // Uglifyjs
            ...
            if (!data) {
                // Data wasn't given by a previous step
                data = fs.readFileSync(filePath);
            }
