...
exports.static = function(directory, options) {
    ...
    function readFile(urlPath) {
        ...
        } else if (['.elm', '.js'].includes(extension)) {
    ...
    function readJavaScriptFile(filePath) {
        ...
        if (extension === '.elm') {
            ...
            // Elm
            try {
                data = elm.compileToStringSync([filePath], { cwd: directory });
            } catch(err) {
                // Elm can't compile, so just pass the elm file along
                console.warn(`Elm skipping ${filePath}:\n    ${err}`);
            }
        }
