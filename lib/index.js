...
exports.static = function static(directory, options) {
    ...
    function cacheFile(filePath, stat) {
        ...
        if (ext === '.js' && !shouldIgnore(relativeUrl, options.jsx.ignore)) {
            ...
            memoryFileSystem.mkdirpSync(onlyPath);
            memoryFileSystem.writeFileSync(filePath, data);
            ...
            compiler.resolvers.normal.fileSystem = memoryFileSystem;
            compiler.resolvers.context.fileSystem = memoryFileSystem;

            compiler.run(function(err, stats) {
                if (err || stats.hasErrors()) {
                    console.warn(`Webpack skipping ${filePath}:\n    ${err}`);
                    data = memoryFileSystem.readFileSync(filePath).toString();
                    return;
                }

                data = stats.compilation.assets[filePath].source();
            });
        }
