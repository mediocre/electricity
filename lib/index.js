...
exports.static = function(directory, options) {
    ...
    /**
     * Parses an HTTP request with a path potentially containing a hash (/robots-3f54004ef6fc21b24a9e6069fc114fd9070b77a1.txt)
    ...
    function parseRequest(req) {
        ...
        const hashMatch = req.path.match(hashRegex);
        const hash = hashMatch ? hashMatch[0].slice(1).replace(/\.([^.]*$)/, '') : '';

        return {
            hash: hash.length == 40 ? hash : null,
            path: hash.length == 40 ? req.path.replace(hashRegex, '$1') : req.path
        };
    }
    ...
    /**
     * Converts a URL (/robots.txt) to a local file path (/Users/username/site/public/robots.txt).
     * @param {string} url
     */
    ...
    return function staticMiddleware(req, res, next) {
        ...
        // Ensure we could read the specified file
        if (!file) {
            // Allow other middleware a chance to handle this request (or ultimately return a 404)
            return next();
        }
