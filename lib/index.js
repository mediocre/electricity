...
exports.static = function static(directory, options) {
    ...
    function absoluteToExpress(filePath) {
        ...
        return prefixSlash(relativePath).replace(/\\/g, '/');
    }
    ...
    function cacheFile(filePath, stat) {
        ...
        var relativePath = absoluteToExpress(filePath);

        var data;

        // SASS compilation
        if (path.extname(relativePath) === '.scss') {
    ...
    watch.createMonitor(directory, function monitorFiles(monitor) {
        ...
        monitor.on('removed', function fileRemoved(f, stat) {
            var relativePath = absoluteToExpress(f);
