...
exports.static = function(directory, options) {
    ...
    // Disable eager loading by default
    if (!options.eager) {
        options.eager = {
            enabled: false,
            jitter: {
    ...
    /**
     * Eagerly fetch all files in the specified directory (recursively).
     * @param {string} filePath
     */
     function fetchDirectory(filePath) {
        fs.readdir(filePath, { withFileTypes: true }, function(err, files) {
            ...
            files.forEach(file => {
                setTimeout(() => {
                    const _filePath = path.join(filePath, file.name);

                    if (file.isDirectory()) {
                        return fetchDirectory(_filePath);
                    }

                    fetchFile(toUrlPath(_filePath));
                }, randomNumber(options.eager.jitter.min, options.eager.jitter.max));
            });
        });
    }
    ...
    // Eagerly fetch all files
    if (options.eager.enabled) {
