    ...
    function cacheFile(filePath, stat) {
        ...
        if (!stat.isDirectory()) {
            ...
            if (path.extname(relativeUrl) === '.scss' && !shouldIgnore(relativeUrl, options.sass.ignore)) {
                ...
                data = options.uglifycss.enabled ? UglifyCss.processString(data, options.uglifycss) : data;
            } else if (type === 'application/javascript' || type === 'text/javascript') {
                if (!shouldIgnore(relativeUrl, options.snockets.ignore)) {
                    try {
                ...
                if (options.uglify.enabled && !shouldIgnore(relativeUrl, options.uglify.ignore)) {
                    if(data) {
                        options.uglify.fromString = true;
                        data = Uglify.minify(data.toString(), options.uglify).code;
                    } else {
                        options.uglify.fromString = false;
            ...
            } else if (type === 'text/css' && options.uglifycss.enabled) {
                data = UglifyCss.processFiles([filePath], options.uglifycss);
            } else {
                data = fs.readFileSync(filePath);
            }
