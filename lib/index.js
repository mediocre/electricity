...
// For some reason this refuses to include normally
var sassGraph = require('../node_modules/sass-graph/sass-graph');
...
exports.static = function static(directory, options) {
    ...
    if (options.sassImagePath) {
        if (typeof options.sassImagePath !== 'string') {
            throw Error('Image Path must be a string');
        } else if (options.sassImagePath.slice(-1) === '/') {
            options.sassImagePath = options.sassImagePath.slice(0, options.sassImagePath.length - 1);
        }
    }
    ...
    function cacheFile(filePath, stat) {
        ...
        if (!stat.isDirectory()) {
            ...
            // SASS compilation
            if (path.extname(relativeUrl) === '.scss') {
                data = sass.renderSync({
                    file: filePath,
                    imagePath: options.sassImagePath || ''
                });
    ...
    // Build a SASS dep graph for updating compiled includes
    var sassDeps = sassGraph.parseDir(directory);

    function reloadWithDependents(filePath, stat) {
        if(path.extname(filePath) === '.scss') {
            if(sassDeps.index[filePath]) {
                sassDeps.index[filePath].importedBy.forEach(reloadWithDependents);
            }
        }
        cacheFile(filePath, stat);
    }
    ...
    watch.createMonitor(directory, function monitorFiles(monitor) {
        monitor.on('changed', reloadWithDependents);
        monitor.on('created', reloadWithDependents);

        monitor.on('removed', function removeFile(f, stat) {
            files[toRelativeUrl(f)] = null;
        });
    });
